    ...
    const handleExportCSV = async () => {
        ...
        try {
            ...
            const [receivedMoneySnap, sentMoneySnap, currentExpensesSnap] =
                await Promise.all([
                    getDocs(query(collection(db, "payments"), where("fromUserId", "==", uid))),
                    getDocs(query(collection(db, "payments"), where("toUserId", "==", uid))),
    ...
    return (
        <div className="dashboard-container">
            <GreetingSection
                ...
                exporting={exporting}
            />
            <GroupActivitySection groupIds={groupIds} />
        </div>
    );
};
